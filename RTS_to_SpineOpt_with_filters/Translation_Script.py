...
def _extract_relationships(source_relationship_class):
    x_to_move = []
...
RELATIONSHIP_PARAMETER_VALUES = export_relationship_parameter_values(in_db)
def _extract_relationship_parameter_values(parameters, source_class):
    x_to_move = []
    for value in RELATIONSHIP_PARAMETER_VALUES:
        if value[P.CLASS] == source_class and value[P.NAME] in list(parameters):
...
def add_relationships(relationship_class):
    relationship_list = _extract_relationships(relationship_class)
    for rel in relationship_list:
        import_relationships(out_db, [(relationship_class, rel)])
...
def get_parameter_values_to_add_relationship(parameter_name,relationship_class):
    return(dict(_extract_relationship_parameter_values([parameter_name],relationship_class)))
...
def add_modified_parameter_relationship(base_dict,parameter_name,input_parameter_names,operation,object_class):
    aliases = {}
    for input_parameter_index in range(len(input_parameter_names)):
        input_parameter = input_parameter_names[input_parameter_index]
        aliases[input_parameter] = object_class+str(input_parameter_index)
        locals()[object_class+str(input_parameter_index)]=get_parameter_values_to_add_relationship(input_parameter,object_class)
        for i in range(len(operation)):
            operation[i] = operation[i].replace(input_parameter,object_class+str(input_parameter_index)+'_at_key')
    to_add = get_parameter_values_to_add_relationship(input_parameter_names[0],object_class)
...
def create_relationships(relationship_class):
    print("adding relationships of class "+relationship_class)
    add_relationships(relationship_class)

    to_add = {}
    translation_parameters = read_translation_file(relationship_class)

    for i in translation_parameters:
        to_add = add_modified_parameter_relationship(to_add,translation_parameters[i][0],translation_parameters[i][1],translation_parameters[i][2],relationship_class)
        #print("added "+str(translation_parameters[i][0]))

    parameter_names = list(to_add.keys())
    parameter_names_to_add = [tuple([relationship_class]+[parameter_name]) for parameter_name in parameter_names]
    import_relationship_parameters(out_db, parameter_names_to_add)
    ...
    for parameter_name in parameter_names:
        for object_name in to_add[parameter_name]:
            for value_index in range(len(to_add[parameter_name][object_name])):
                value = to_add[parameter_name][object_name][value_index]
                to_append = [relationship_class,object_name,parameter_name,value,"alternative"+str(value_index)]
                parameter_values_to_add.append(tuple(to_append))
    import_relationship_parameter_values(out_db, parameter_values_to_add)

    print("added all relationships of class "+relationship_class)

try:       
    create_objects("node")
    create_objects("unit")
    create_objects("node_group")
    create_objects("connection")
    create_objects("commodity")
    create_relationships("connection__from_node")
    create_relationships("connection__to_node")
    create_relationships("connection__node_node")
    create_relationships("unit__from_node")
    create_relationships("unit__to_node")
    create_relationships("unit__node_node")
